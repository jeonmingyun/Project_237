...
class IntroTabFragment : Fragment() {
    ...
    private fun setLayoutItem(dataList: ArrayList<Any>) {
        binding.introImg.setImageResource(dataList[0].toString().toInt()) // 인트로 이미지
        binding.introContent.text = dataList[1].toString() // 인트로 내용글
        binding.capitalImg.setImageResource(dataList[2].toString().toInt()) // 수도 이미지
    }
}